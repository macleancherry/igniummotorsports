...
export async function onRequestGet(context: Context) {
  ...
  const drivers = await context.env.DB.prepare(
...
export async function onRequestPost(context: Context) {
  ...
  if (Array.isArray(body.driverIds)) {
    const event = await context.env.DB.prepare("SELECT id FROM events WHERE slug = ?").bind(body.slug).first<any>();
    if (event?.id) {
      await context.env.DB.prepare("DELETE FROM event_drivers WHERE event_id = ?").bind(event.id).run();
